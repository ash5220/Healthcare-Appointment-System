...
Curato is a production-focused web application that streamlines the appointment lifecycle between patients and providers. The platform includes JWT auth with refresh-token rotation, MFA flows, role-based access control, PHI audit logging, encrypted sensitive data, and an Angular SPA.
...
All demo accounts use the password **`Demo1234!`**. No registration required — log in and explore immediately.

| Email | Role | What to explore |
| -------------------------------------------- | ------- | ----------------------------------------------- |
| `demo.patient.alice@infinitesevens.com` | Patient | Appointments, medical records, insurance, messages |
| `demo.patient.bob@infinitesevens.com` | Patient | Appointments, insurance, messages |
| `demo.patient.carol@infinitesevens.com` | Patient | Upcoming appointments |
| `demo.dr.sarah@infinitesevens.com` | Doctor | Cardiology — confirmed appointments, availability |
| `demo.dr.michael@infinitesevens.com` | Doctor | General Practice — scheduled appointments |
| `demo.dr.priya@infinitesevens.com` | Doctor | Dermatology — completed appointment with record |
| `demo.dr.robert@infinitesevens.com` | Doctor | Pediatrics — completed appointment with record |
| `demo.admin@infinitesevens.com` | Admin | User management, doctor approvals, system stats |

> Demo data can be refreshed at any time from the GitHub Actions tab using the **Reset Demo Data** workflow.

### Demo Video


 ![curato demo](https://github.com/user-attachments/assets/de53e2ee-1880-4eb9-9621-f10f3b7e756b)
...
- Request and confirm email changes securely
...
- Generate administrative reports and view system metrics
...
- Real-time system notifications for critical events
- In-app alerts and toast notifications for user interactions
...
The system uses a highly normalized relational model with strict referential integrity (e.g., `RESTRICT` constraints) to ensure HIPAA-compliant data retention.
Key models include:
