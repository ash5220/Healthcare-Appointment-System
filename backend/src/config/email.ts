 ...
 */
export const initializeEmailTransporter = async (): Promise<Transporter> => {
    if (transporter) return transporter;

    const hasSmtpConfig = env.smtpHost && env.smtpUser && env.smtpPassword;
    ...
    if (hasSmtpConfig) {
        ...
        logger.info(`📧 Email transporter configured with SMTP host: ${env.smtpHost}`);
...
export const sendEmail = async (options: EmailOptions): Promise<EmailResult> => {
    try {
        ...
        const mailOptions = {
            from: options.from || `"Healthcare System" <${env.smtpUser || 'noreply@healthcare.local'}>`,
