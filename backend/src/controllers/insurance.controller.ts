...
export const createInsurance = asyncHandler<AuthenticatedRequest>(async (req, res) => {
    const insurance = await insuranceService.create(req.user.userId, req.body as CreateInsuranceInput);
...
export const updateInsurance = asyncHandler<AuthenticatedRequest>(async (req, res) => {
    const insurance = await insuranceService.update(req.params.id, req.user.userId, req.body as UpdateInsuranceInput);
...
export const verifyInsurance = asyncHandler(async (req, res) => {
    const insurance = await insuranceService.verify(req.params.id, req.body as VerifyInsuranceInput);
    successResponse(res, { insurance }, `Insurance verification status updated to: ${insurance.verificationStatus}`);
});
