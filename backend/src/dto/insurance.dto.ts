...
export const createInsuranceValidation = z.object({
  body: z
    .strictObject({
      ...
      copayAmount: z.number().nonnegative('copayAmount must be non-negative').optional(),
      deductibleAmount: z.number().nonnegative('deductibleAmount must be non-negative').optional(),
    })
...
export const verifyInsuranceValidation = z.object({
  ...
  body: z.strictObject({
    status: z.enum(Object.values(InsuranceStatus) as [string, ...string[]], {
