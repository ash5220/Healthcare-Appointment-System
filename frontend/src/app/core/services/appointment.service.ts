...
import { Appointment, AppointmentStatus, CreateAppointmentData, AppointmentFilters, PaginatedResponse, AppointmentResponse } from '../models';
...
export class AppointmentService {
    ...
    readonly pastAppointments = computed(() =>
        this.appointmentsSignal().filter(
            (apt) => apt.status === AppointmentStatus.COMPLETED || apt.status === AppointmentStatus.NO_SHOW
        )
    );
    ...
    getAvailableSlots(doctorId: string, date: string): Observable<{ data: { slots: string[] } }> {
        ...
        return this.http.get<{ data: { slots: string[] } }>(`${this.apiUrl}/available-slots`, { params });
    }
    ...
    cancelAppointment(id: string, cancellationReason: string): Observable<AppointmentResponse> {
        return this.http.post<AppointmentResponse>(`${this.apiUrl}/${id}/cancel`, { cancellationReason }).pipe(
            tap((response) => {
                const currentAppointments = this.appointmentsSignal();
                const index = currentAppointments.findIndex((apt) => apt.id === id);
                if (index !== -1) {
    ...
    completeAppointment(
        ...
        return this.http.post<AppointmentResponse>(`${this.apiUrl}/${id}/complete`, { notes, prescriptions });
    }
