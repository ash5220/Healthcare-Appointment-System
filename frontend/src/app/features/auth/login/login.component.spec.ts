...
describe('LoginComponent', () => {
    ...
    describe('login submission', () => {
        ...
        const mockSuccessResponse = (role: UserRole) => ({
            success: true,
            data: {
                user: {
                    id: '1',
        ...
        it('should redirect to patient dashboard for patient role', fakeAsync(() => {
            mockAuthService.login.and.returnValue(of(mockSuccessResponse(UserRole.PATIENT)));
            (mockAuthService as any).currentUser = signal({ role: UserRole.PATIENT });
        ...
        it('should redirect to doctor dashboard for doctor role', fakeAsync(() => {
            mockAuthService.login.and.returnValue(of(mockSuccessResponse(UserRole.DOCTOR)));
            (mockAuthService as any).currentUser = signal({ role: UserRole.DOCTOR });
            const routerSpy = spyOn(component['router'], 'navigate');
        ...
        it('should redirect to admin dashboard for admin role', fakeAsync(() => {
            mockAuthService.login.and.returnValue(of(mockSuccessResponse(UserRole.ADMIN)));
            (mockAuthService as any).currentUser = signal({ role: UserRole.ADMIN });
            const routerSpy = spyOn(component['router'], 'navigate');
